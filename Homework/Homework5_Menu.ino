...
void setManualControl() {
  if(flagLedManual){
    ...
    if (color > 3){
      Serial.println(F("Choose a valid number! I don't have a color for this number! Not that hard!"));
      setManualControl();
    }
    else if(color!=-1){
...
void decisionMainMenu() {
  if (flagMenu) { //if it is on the main menu, get's users choice, decides on which submenu it goes
    if (Serial.available()) {
...
void decisionSensorSubMenu() { //decides on the choice of the sensor submenu
...
void decisionResetMenu() { //decides on the choice of the reset submenu
  if (flagResetMenu) {
    if (Serial.available()) {
...
void decisionSystemMenu() { //decides on the choice of the system submenu
  if (flagSystemMenu) {
    if (Serial.available()) {
...
void decisionLedMenu() { //decides on the choise of the led submenu
  if (flagLedMenu) {
    if (Serial.available()) {
